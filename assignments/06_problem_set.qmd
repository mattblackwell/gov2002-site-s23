...
d. Interpret your findings in parts b) and c). How does the central limit theorem explain what you see in the simulations?
...
b. Use the central limit theorem to approximate the probability (in `R`) that $T$ is less than or equal to 1.
...
(b) Find $\text{Var}(X_n)$. Does the variance of the sequence grow or shrink as $n$ grows?

(c) Use the definition of convergence in probability to show that $X_n \xrightarrow{p} 0$. 
...
$$
\begin{aligned}
f_X = \frac{1}{2}(1 + \theta x), \quad-1 < x < 1, \ -1 < \theta < 1.
\end{aligned}
$$
