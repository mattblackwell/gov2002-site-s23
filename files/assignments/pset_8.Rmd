...
\[
Y = \mathbf{X}^T\bm{\beta}  + u
\]
Show that if this model is true and $E[u|\mathbf{X}] = 0$, then $E[Y|\mathbf{X}] = \mathbf{X}^T \bm{\beta}$.

(Note that this is the opposite of what we showed in lecture, where we saw that if we assume $E[Y|\mathbf{X}] = \mathbf{X}^T \bm{\beta}$, then the conditional mean zero assumption holds, $E[u|\mathbf{X}] = 0$.)
